...
def test_short_explicit_new_topic_question_does_not_reuse_old_context(tmp_path) -> None:
    kb_dir = tmp_path / "kb"
    kb_dir.mkdir(parents=True, exist_ok=True)
    (kb_dir / "licensing_taxi_licence.md").write_text(
        "\n".join(
            [
                ...
                "- Taxi licence applications are required before carrying passengers.",
                "key_points:",
                "- Taxi licence requirements include proof of identity.",
            ]
        ),
        encoding="utf-8",
    )
    (kb_dir / "bins_bulky_waste.md").write_text(
        "\n".join(
            [
                "title: Bulky waste collections",
                "tags: [bins, bulky-waste, collection]",
                "summary:",
                "- Bulky waste collection is available by booked appointment.",
                "key_points:",
                "- Bulky waste collection charges apply per booking.",
            ]
        ),
        encoding="utf-8",
    )
    ...
    second = engine.process_turn(
        session_id=session_id, transcript="How much is bulky waste collection?"
    )
    query_used = str(second.outcome["rag"].get("query_used", ""))
    assert "topic context:" not in query_used.lower()
    assert "bulky waste" in second.response_text.lower()
